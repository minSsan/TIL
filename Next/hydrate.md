...
> 스크롤 값이 변화함에 따라, 애니메이션을 적용할 DOM에 접근하는 과정 _(useRef 사용)_ 에서 오류가 발생한 것으로 추측된다.  
...
- 각 페이지들을 최소한의 JS 코드와 함께 **HTML로 변환**하여 서버에 저장해둔다.
- 페이지가 **처음으로** 브라우저에 로드 될 때, 앞서 미리 변환해둔 HTML 파일을 브라우저에 띄운다. _**(Initial Load - SSR)**_
- 이후에는 클라이언트에서 해당 페이지의 JS 파일 _(= 리액트 파일)_ 이 로드되면서 리액트 컴포넌트가 초기화되고, 완전한 페이지가 구성된다. _**(Hydration)**_

<br>

## 👉🏻 **Hydration**

위에서 말한 **_"리액트 컴포넌트가 초기화되고, 페이지가 interactive해지는 과정"_** 을 `hydration` 이라고 한다.

> _initial load에서 로드된 HTML 파일은 불완전했다면, `hydration`이 완료된 페이지는 **페이지 간의 이동 등의 기능들**이 모두 동작하는 페이지를 말한다._
>
...
>
> 따라서, `Link 태그`도 클라이언트 사이드의 JS 코드가 로드된 이후에야 동작한다. ( [👉🏻 next 공식 문서](https://nextjs.org/learn/basics/data-fetching/pre-rendering) )
>
> ❓ _그런데 **next/link**의 **`Link 태그`에 등록된 페이지**는 **pre-rendering** 된다는 포스팅을 발견했는데, 해당 내용은 더 알아봐야 할 것 같다._ ( [👉🏻 Next Link](https://hun1846.medium.com/next-js-link-db4a183a68a0) )
